...
export async function reindexDocument(request, response, next) {
  try {
    const doc = await knowledgeService.reindexDocument(request.params.id, request.auth.organizationId);
...
export async function deleteDocument(request, response, next) {
  try {
    const result = await knowledgeService.deleteDocument(request.params.id, request.auth.organizationId);
    return sendSuccess(response, 200, result, request.requestId);
  } catch (error) {
    return next(error instanceof AppError ? error : new AppError(error.statusCode || 502, error.code || 'KNOWLEDGE_SERVICE_ERROR', error.message));
  }
...
export async function getDocument(request, response, next) {
  try {
    const doc = await knowledgeService.getDocument(request.params.id, request.auth.organizationId);
    return sendSuccess(response, 200, doc, request.requestId);
  } catch (error) {
    return next(error instanceof AppError ? error : new AppError(error.statusCode || 502, error.code || 'KNOWLEDGE_SERVICE_ERROR', error.message));
  }
...
export async function searchKnowledge(request, response, next) {
  try {
    ...
    const result = await knowledgeService.search({ q, mode, topK, documentType, organizationId: request.auth.organizationId });
    return sendSuccess(response, 200, result, request.requestId);
  } catch (error) {
    return next(error instanceof AppError ? error : new AppError(error.statusCode || 502, error.code || 'KNOWLEDGE_SERVICE_ERROR', error.message));
  }
...
export async function getEmployeeKnowledgeInsights(request, response, next) {
  try {
    const result = await knowledgeService.getEmployeeKnowledgeInsights(request.params.employeeId, request.auth.organizationId);
    return sendSuccess(response, 200, result, request.requestId);
  } catch (error) {
    return next(error instanceof AppError ? error : new AppError(error.statusCode || 502, error.code || 'KNOWLEDGE_SERVICE_ERROR', error.message));
  }
