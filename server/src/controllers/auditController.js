...
export async function listAuditLog(req, res, next) {
  try {
    ...
    const result = await auditService.listAuditLog(req.auth.organizationId, {
...
export async function exportAuditLog(req, res, next) {
  try {
    ...
    const csv = await auditService.exportAuditLogCsv(req.auth.organizationId, { action, entityType, entityId, userId, startDate, endDate });
...
export async function getActivityTimeline(req, res, next) {
  try {
    ...
    const result = await auditService.listActivityTimeline(req.auth.organizationId, {
