...
export async function listAttachments(req, res, next) {
  try {
    ...
    const result = await attachmentService.listForEntity(req.auth.organizationId, entityType, entityId);
...
export async function downloadAttachment(req, res, next) {
  try {
    const attachment = await attachmentService.getById(req.params.id, req.auth.organizationId);
