...
const STATUS_STYLES = {
  ...
  ESCALATED: 'bg-rose-500/10 text-rose-400 border-rose-500/30',
};
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="w-full max-w-md p-6 bg-slate-900 border border-slate-800 rounded-3xl" onClick={(e) => e.stopPropagation()}>
        <h2 className="text-lg font-bold text-slate-100 mb-4">New Task</h2>
        <div className="space-y-3">
          ...
          <input placeholder="Owner User ID (optional)" value={form.ownerUserId} onChange={(e) => setForm({ ...form, ownerUserId: e.target.value })} className="w-full px-3 py-2 text-xs bg-slate-950 border border-slate-800 rounded-lg text-slate-200" />
      ...
      <div className="w-full max-w-lg h-full bg-slate-900 border-l border-slate-800 overflow-y-auto p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <h2 className="text-lg font-bold text-slate-100 mt-2">{task.title}</h2>
            <p className="text-xs text-slate-500">{task.priority} priority · Owner: {task.ownerUserId?.name || 'Unassigned'}</p>
          </div>
          <button onClick={onClose} className="text-slate-500 hover:text-slate-300">✕</button>
        </div>

        {task.description && <p className="text-xs text-slate-400 mb-4">{task.description}</p>}
        {error && <p className="text-xs text-rose-400 mb-3">{error}</p>}

        {!closed && (
          <div className="space-y-2 mb-4">
            <div className="flex gap-2">
              <input placeholder="Assign/Reassign User ID" value={reassignId} onChange={(e) => setReassignId(e.target.value)} className="flex-1 px-2 py-1.5 text-[11px] bg-slate-950 border border-slate-800 rounded-lg text-slate-200" />
            ...
            <div className="flex gap-2">
              <input placeholder="Escalate to User ID" value={escalateId} onChange={(e) => setEscalateId(e.target.value)} className="flex-1 px-2 py-1.5 text-[11px] bg-slate-950 border border-slate-800 rounded-lg text-slate-200" />
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => run(() => taskService.cancel(id))} className="px-3 py-1.5 text-[10px] font-semibold bg-slate-700 hover:bg-slate-600 text-white rounded-lg">Cancel</button>
            </div>
          </div>
        )}

        <div className="mb-4">
          <h3 className="text-xs font-bold text-slate-300 mb-2">History</h3>
          <div className="space-y-1.5">
            {task.history.map((h, i) => (
              <div key={i} className="text-[11px] text-slate-400">
                <span className="font-semibold text-slate-300">{h.action}</span> — {new Date(h.at).toLocaleString()} {h.note && `· ${h.note}`}
              </div>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-extrabold text-slate-100">Tasks</h1>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden">
        ...
        {!loading && tasks.length === 0 && <p className="text-xs text-slate-500 italic p-6 text-center">No tasks found.</p>}
        {tasks.map((t) => (
          <button key={t._id} onClick={() => setSelectedId(t._id)} className="w-full text-left flex items-center justify-between px-5 py-3 border-b border-slate-800 last:border-0 hover:bg-slate-800/50">
            <div>
              <p className="text-sm font-semibold text-slate-200">{t.title}</p>
              <p className="text-[11px] text-slate-500">{t.priority} · Owner: {t.ownerUserId?.name || 'Unassigned'} {t.dueDate && `· Due ${new Date(t.dueDate).toLocaleDateString()}`}</p>
            </div>
