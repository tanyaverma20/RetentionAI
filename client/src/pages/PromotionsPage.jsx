  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-3xl bg-white border border-slate-100 shadow-card">
        <div>
          <div className="flex items-center gap-2 text-xs font-mono font-semibold text-rose-600 uppercase tracking-widest mb-1">
            <span>HR Operations</span><span>•</span><span>{total} Records</span>
          </div>
          <h1 className="text-3xl font-extrabold text-slate-900">Promotion History</h1>
          <p className="text-sm text-slate-500 mt-1">Track employee role changes and salary adjustments.</p>
        </div>
        {canManage && (
      ...
      {showForm && canManage && (
        <form onSubmit={handleSubmit} className="p-6 bg-white border border-slate-100 rounded-3xl space-y-4 shadow-card">
          <h2 className="text-lg font-bold text-slate-900">Add Promotion Record</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <input required placeholder="Previous Role" value={form.previousRole} onChange={(e) => setForm({...form, previousRole: e.target.value})} className="px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-slate-800 text-xs" />
            <input required placeholder="New Role" value={form.newRole} onChange={(e) => setForm({...form, newRole: e.target.value})} className="px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-slate-800 text-xs" />
            <input type="date" required value={form.promotionDate} onChange={(e) => setForm({...form, promotionDate: e.target.value})} className="px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-slate-800 text-xs" />
            <input type="number" min="0" required placeholder="Salary Increase %" value={form.salaryIncreasePercentage} onChange={(e) => setForm({...form, salaryIncreasePercentage: e.target.value})} className="px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-slate-800 text-xs" />
            <select required value={form.approvedBy} onChange={(e) => setForm({...form, approvedBy: e.target.value})} className="px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-slate-800 text-xs">
              <option value="">Approved By (Manager)</option>
              {(employees || []).map((emp) => <option key={emp._id || emp.id} value={emp._id || emp.id}>{emp.firstName} {emp.lastName}</option>)}
            </select>
            <input placeholder="Reason / Comments" value={form.reason} onChange={(e) => setForm({...form, reason: e.target.value})} className="sm:col-span-2 lg:col-span-3 px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-slate-800 text-xs" />
          </div>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="p-12 text-center bg-white border border-slate-100 rounded-3xl">
            <h3 className="text-lg font-bold text-slate-800">No promotion records found</h3>
            <p className="text-xs text-slate-500 mt-1">Record a promotion to get started.</p>
          </div>
        ) : records.map((r) => (
          <div key={r._id || r.id} className="p-6 bg-white border border-slate-100 rounded-3xl shadow-card flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900">{r.employeeId?.firstName || r.employeeId || '—'} {r.employeeId?.lastName || ''}</h3>
                <div className="flex items-center gap-2 mt-1 text-xs font-mono text-slate-500">
                  ...
                  <span>→</span>
                  <span className="text-indigo-600 font-bold">{r.newRole}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-wrap md:flex-nowrap items-center gap-6">
              <div className="text-xs text-slate-500">
                <div>Date</div>
                <div className="font-semibold text-slate-800">{fmt(r.promotionDate)}</div>
              </div>
              <div className="text-xs text-slate-500">
                <div>Salary Increase</div>
                <div className="font-bold text-emerald-600">+{r.salaryIncreasePercentage}%</div>
              </div>
              {r.reason && (
              ...
              {canManage && (
                <button onClick={() => dispatch(deleteHrRecord({ collection: 'promotions', id: r._id || r.id }))}
                  className="p-2 text-slate-500 hover:text-rose-600 bg-slate-100 hover:bg-slate-100 rounded-xl transition-colors ml-4 shrink-0" title="Delete">
