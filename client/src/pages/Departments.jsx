    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-6 rounded-3xl bg-white border border-slate-100 shadow-card">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-900">Department Management</h1>
        ...
        {canManage && (
          <div className="flex gap-3">
            <button
              ...
              className="flex items-center justify-center gap-2 px-5 py-3 text-sm font-semibold text-indigo-600 bg-indigo-50 border border-indigo-100 hover:bg-indigo-50 rounded-2xl transition-all transform hover:-translate-y-0.5"
            >
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-5 py-3 text-sm font-semibold text-rose-600 bg-rose-50 hover:bg-rose-50 border border-rose-100 rounded-2xl transition-all"
            >
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {successMessage && (
        <div className="flex items-center justify-between p-4 bg-emerald-50 border border-emerald-100 rounded-2xl text-emerald-600 text-sm">
          ...
          <button onClick={() => dispatch(clearDepartmentSuccess())} className="text-emerald-600 hover:text-emerald-600 font-bold">
            ×
          </button>
        </div>
      )}
      ...
      {error && (
        <div className="flex items-center justify-between p-4 bg-rose-50 border border-rose-100 rounded-2xl text-rose-600 text-sm">
          <span>{error}</span>
          <button onClick={() => dispatch(clearDepartmentError())} className="text-rose-600 hover:text-rose-600 font-bold">
            ×
          </button>
        </div>
      )}
      ...
      {/* Search & Filter Bar */}
      <div className="flex items-center justify-between gap-4 p-4 bg-white border border-slate-100/80 rounded-2xl backdrop-blur-md">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-slate-50 border border-slate-100 focus:border-indigo-500 rounded-xl text-slate-900 text-sm placeholder-slate-500 focus:outline-none transition-colors"
          />
        </div>
      </div>
          ...
          {departments.map((dept) => {
            ...
            return (
              <div
                ...
                className="group relative flex flex-col justify-between p-6 bg-white border border-slate-100 hover:border-indigo-100 rounded-3xl shadow-lg hover:shadow-card transition-all duration-300"
              >
                <div>
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-indigo-600/30 to-violet-600/30 border border-indigo-100 flex items-center justify-center text-indigo-600 font-black text-lg font-mono shadow-inner">
                        {dept.code}
                      </div>
                      <div>
                    ...
                    <span className="px-2.5 py-1 text-xs font-mono font-bold text-indigo-600 bg-indigo-50 border border-indigo-100 rounded-full">
                      {dept.employeeCount || 0} Staff
                    </span>
                  </div>

                  <p className="text-xs text-slate-500 line-clamp-2 min-h-[2.5rem] mb-4">
                ...
                <div className="pt-4 border-t border-slate-100/80 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div className="text-xs">
                      ...
                      <div className="text-slate-400 font-mono">Department Manager</div>
                    </div>
                  </div>
          ...
          <div className="w-full max-w-md p-6 bg-white border border-slate-100 rounded-2xl shadow-card space-y-4">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Are you sure you want to delete <span className="text-slate-800 font-bold">{deleteConfirmDept.name}</span>?
      ...
      {isDeleteAllOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-sm animate-fade-in">
          <div className="w-full max-w-md p-6 bg-white border border-rose-100 rounded-2xl shadow-card space-y-4">
            ...
            <div className="text-center">
              <h3 className="text-lg font-bold text-slate-900">Delete All Departments</h3>
              <p className="text-xs text-slate-500 mt-1">
                This will permanently delete <span className="text-slate-800 font-bold">all {departments.length} department(s)</span>.
