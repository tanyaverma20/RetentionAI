...
const STATUS_COLORS = {
  ...
  ON_LEAVE: 'bg-violet-50 text-violet-600 border-violet-100',
  HOLIDAY: 'bg-sky-50 text-sky-600 border-sky-100',
};
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-3xl bg-white border border-slate-100 shadow-card">
        <div>
          <div className="flex items-center gap-2 text-xs font-mono font-semibold text-emerald-600 uppercase tracking-widest mb-1">
            <span>HR Operations</span><span>•</span><span>{total} Records</span>
          </div>
          <h1 className="text-3xl font-extrabold text-slate-900">Attendance Management</h1>
          <p className="text-sm text-slate-500 mt-1">Track daily attendance, clock-in/out times, and leave records.</p>
        </div>
        {canManage && (
      ...
      <div className="p-4 bg-white/80 border border-slate-100 rounded-3xl backdrop-blur-md">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          <select value={filterEmployee} onChange={(e) => { setFilterEmployee(e.target.value); dispatch(setHrPage(1)); }}
            className="w-full px-3.5 py-2 bg-slate-50 border border-slate-100 focus:border-indigo-500 rounded-xl text-slate-800 text-xs">
      ...
      {showForm && canManage && (
        <form onSubmit={handleSubmit} className="p-6 bg-white border border-slate-100 rounded-3xl space-y-4 shadow-card">
          <h2 className="text-lg font-bold text-slate-900">Record Attendance</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <select required value={form.employeeId} onChange={(e) => setForm({...form, employeeId: e.target.value})}
              className="px-3 py-2 bg-slate-50 border border-slate-100 rounded-xl text-slate-800 text-xs">
      ...
      <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-card">
        {loading ? (
          ...
          <div className="p-12 text-center">
            ...
            <p className="text-xs text-slate-500 mt-1">Try adjusting filters or add new records.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-100/60">
                {records.map((r) => (
                  <tr key={r._id || r.id} className="hover:bg-slate-100/40 transition-colors">
                    ...
                    <td className="px-6 py-4 text-xs text-slate-500">{r.workMode}</td>
                    {canManage && (
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-4 bg-slate-50 border-t border-slate-100 text-xs text-slate-500">
            <div>Page <span className="font-bold text-slate-800">{page}</span> of <span className="font-bold text-slate-800">{totalPages}</span> ({total} records)</div>
