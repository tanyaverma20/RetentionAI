...
export default function ReportsPage() {
  ...
  return <div className="space-y-6"><header className="p-6 rounded-3xl bg-white border border-slate-100 shadow-card"><p className="text-xs font-mono font-semibold uppercase tracking-widest text-indigo-600">Reporting centre</p><h1 className="mt-1 text-3xl font-extrabold text-slate-900">Workforce Reports</h1><p className="mt-2 text-sm text-slate-500">Download CSV reports. PDF export remains API-ready for the next delivery.</p></header><div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">{reports.map(([slug, title, description]) => <article key={slug} className="p-6 rounded-3xl bg-white border border-slate-100"><h2 className="font-bold text-slate-900">{title}</h2><p className="mt-2 text-sm text-slate-500 min-h-10">{description}</p><button onClick={() => download(slug)} className="mt-5 px-4 py-2 text-sm font-semibold rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white">Export CSV</button></article>)}</div></div>;
}
