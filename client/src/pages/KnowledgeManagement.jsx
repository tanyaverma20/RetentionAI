...
const STATUS_STYLES = {
  INDEXED: 'bg-emerald-50 text-emerald-600 border-emerald-100',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/80 backdrop-blur-sm animate-fade-in">
      <div className="w-full max-w-md p-6 bg-white border border-slate-100 rounded-2xl shadow-card space-y-4">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-1.5">File (PDF, DOCX, TXT, MD)</label>
            <input
              type="file"
              accept=".pdf,.docx,.txt,.md"
              onChange={(e) => setFile(e.target.files[0])}
              className="w-full text-xs text-slate-600 file:mr-3 file:px-3 file:py-1.5 file:rounded-lg file:border-0 file:bg-indigo-600 file:text-white file:text-xs file:font-semibold"
            />
          </div>
          <div>
            ...
            <select
              ...
              className="w-full px-3.5 py-2 bg-slate-50 border border-slate-100 rounded-xl text-slate-800 text-xs focus:outline-none focus:border-indigo-500"
            >
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2 bg-slate-50 border border-slate-100 rounded-xl text-slate-800 text-xs placeholder-slate-500 focus:outline-none focus:border-indigo-500"
            />
          </div>
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-semibold text-slate-500 hover:text-slate-900 bg-slate-100 rounded-xl">
              Cancel
            </button>
  ...
  return (
    <div className="p-6 bg-white border border-slate-100 rounded-3xl shadow-card space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-bold text-slate-900">Knowledge Search</h2>
        <div className="flex bg-slate-50 border border-slate-100 rounded-xl p-1 text-xs">
          {['semantic', 'keyword'].map((m) => (
            <button
              ...
              type="button"
              onClick={() => setMode(m)}
              className={`px-3 py-1.5 rounded-lg font-semibold transition-colors ${mode === m ? 'bg-indigo-600 text-white' : 'text-slate-500'}`}
            >
      ...
      <form onSubmit={handleSearch} className="flex gap-3">
        <input
          ...
          className="flex-1 px-3.5 py-2 bg-slate-50 border border-slate-100 rounded-xl text-slate-900 text-sm placeholder-slate-500 focus:outline-none focus:border-indigo-500"
        />
        <button type="submit" disabled={loading} className="px-5 py-2 text-sm font-semibold text-white bg-indigo-600 hover:bg-indigo-500 rounded-xl disabled:opacity-50">
      ...
      {results && (
        <div className="space-y-2">
          {results.length === 0 && <p className="text-xs text-slate-400 italic text-center py-4">No matches found.</p>}
          {results.map((r, i) => (
            <div key={i} className="p-3 bg-slate-50 border border-slate-100 rounded-xl">
              <div className="flex items-center justify-between mb-1">
                <span className="text-xs font-semibold text-indigo-600">{r.documentName}</span>
                {r.similarityScore != null && (
                  <span className="text-[10px] font-mono text-slate-400">{(r.similarityScore * 100).toFixed(0)}% match</span>
                )}
              </div>
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-6 rounded-3xl bg-white border border-slate-100 shadow-card">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-slate-900">Knowledge Management</h1>
          <p className="text-sm text-slate-500 mt-1">Upload, index, and manage HR policy documents for grounded knowledge lookup.</p>
        </div>
        {canManage && (
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="px-4 py-2.5 text-sm font-semibold text-slate-800 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-2xl disabled:opacity-50 transition-all"
            >
      ...
      <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden shadow-card">
        <div className="p-4 border-b border-slate-100 flex items-center justify-between">
          <h2 className="text-base font-bold text-slate-900">Indexed Documents</h2>
          <select
            value={typeFilter}
            onChange={(e) => setTypeFilter(e.target.value)}
            className="px-3 py-1.5 bg-slate-50 border border-slate-100 rounded-lg text-slate-800 text-xs focus:outline-none"
          >
        ...
        {loading ? (
          ...
          <div className="p-12 text-center text-sm text-slate-500">No documents indexed yet. Upload one to get started.</div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm text-slate-600">
              ...
              <tbody className="divide-y divide-slate-100/60">
                {documents.map((doc) => (
                  <tr key={doc._id} className="hover:bg-slate-100/40 transition-colors">
                    <td className="px-6 py-4 font-semibold text-slate-900">{doc.filename}</td>
                    <td className="px-6 py-4 text-xs text-slate-500">{doc.documentType?.replace(/_/g, ' ')}</td>
                    <td className="px-6 py-4 text-xs text-slate-500">{doc.uploadedBy?.name || 'Unknown'}</td>
                    <td className="px-6 py-4 font-mono text-xs text-slate-600">{new Date(doc.uploadDate).toLocaleDateString()}</td>
                    <td className="px-6 py-4 text-xs text-slate-500">v{doc.version}</td>
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1">
                        {(doc.tags || []).map((t, i) => (
                          <span key={i} className="px-2 py-0.5 text-[10px] font-mono text-slate-600 bg-slate-100 border border-slate-200 rounded-full">{t}</span>
                        ))}
                      </div>
                    </td>
                    <td className="px-6 py-4 text-xs text-slate-500">{doc.chunkCount}</td>
                    <td className="px-6 py-4">
          ...
          <div className="w-full max-w-md p-6 bg-white border border-slate-100 rounded-2xl shadow-card space-y-4">
            ...
            <div className="text-center">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Are you sure you want to delete <span className="text-slate-800 font-bold">{deleteConfirm.filename}</span>? This removes it from the knowledge base permanently.
              </p>
            </div>
            <div className="flex items-center justify-end gap-3 pt-2">
              <button onClick={() => setDeleteConfirm(null)} className="px-4 py-2 text-sm font-semibold text-slate-500 hover:text-slate-900 bg-slate-100 rounded-xl">
                Cancel
              </button>
